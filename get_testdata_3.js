...
const get_data = async () => {
    ...
    let result = await conn.query('select g.id, g.temp_id as event_id, host_id, m.id as user_id, cover from gathering_temp_2 g left join member_temp_2 m on g.host_id=m.member_id;');
    ...
    for (let i = 0; i < result[0].length; i++) {
        ...
        try {
            ...
            let output = await conn.query('INSERT INTO gathering_2 set ?', binding);
    ...
    result = await conn.query('select id, member_id from member_temp_2;');
    ...
    const config = {
        headers: { 'authorization': 'BEARER eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJhMDk2NTY0ZS00OTBkLTQ5OTEtOTI3Yy0yOGUyOGUyNGIxMWEiLCJleHAiOjE5NTAwOTI5ODcsImlhdCI6MTYzNDczMjk4NywiaXNzIjoiaHR0cHM6Ly9lYXRnZXRoZXIuaW8iLCJuYmYiOjE2MzQ3MzI5ODcsInN1YiI6ImFwLXNvdXRoZWFzdC0xOmY0YWY4N2Y4LWIyYmItNDIwYy1hYWUzLTEwMjc2MzJlMTNhNCJ9.95aN96Vk6VH9GQrToL3dA-G_60bKIUNaYFXzFWaCwT_P4IjD4FYWFH6oZJwu2kI_RXU0GsmPog57M7KmXiFJwA' }
    }
    ...
    for (let j = 0; j < result[0].length; j++) {
        ...
        await timeout(500);
        try {
            let member = await axios.get(`https://api.eatgether.io/v4.0/member/profile/${result[0][j].member_id}?language=zh-TW`, config)


            let avatar = await axios.get(`https://api.eatgether.io/v4.0/member/avatar/${result[0][j].member_id}`, config)



            binding = {
                id: result[0][j].user_id,
                username: member.data.member.profile.nickname,
                email: 'test' + result[0][j].id + '@test.com',
                ...
                picture: `https://cdn.eatgether.com/member/${result[0][j].member_id}/avatar/${avatar.data.avatars[0]}`,
                role: 1,
                provider: 'native',
                password: encryptedPWD,
                origin_id: result[0][j].member_id,

            }


            output = await conn.query('INSERT INTO member_2  set ?', binding);
